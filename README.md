...
We are going to use google maps as a tile with our joined senor and location weather data displaying on top.  For google map access from Data Viz you need to get a Google API Id we will use in this step and set in Data Viz.  Do a web search on "google maps API key" and follow the instructions to get a google maps API key.  You may need to login to the google site with a gmail ID to obtain your Google Id for Google Maps.  Alternatively you can create your own visual representation of the joined data as desired.
